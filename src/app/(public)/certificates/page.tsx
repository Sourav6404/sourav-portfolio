    ...
    <div className="max-w-6xl mx-auto px-6 py-12 flex flex-col gap-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {certificates.map((cert) => (
          <div
            key={cert.id}
            className="glass-card p-6 rounded-2xl flex flex-col md:flex-row gap-6 items-start animate-fade-in relative hover:border-indigo-500/30 hover:shadow-[0_0_30px_rgba(99,102,241,0.15)] transition-all duration-300 group/card cursor-pointer"
          >
            {/* Thumbnail Link */}
            <Link 
              href={`/certificates/${cert.id}`} 
              className="w-full md:w-36 h-28 bg-slate-900 border border-white/5 rounded-xl overflow-hidden shrink-0 block cursor-pointer group relative z-10"
            >
              {!cert.thumbnail ? (
            ...
            <div className="flex flex-col gap-4 flex-grow">
              <div className="flex flex-col gap-1">
                ...
                <Link href={`/certificates/${cert.id}`} className="hover:underline block after:absolute after:inset-0 after:z-0">
              ...
              <div className="flex items-center gap-4 mt-2 pt-4 border-t border-white/5 flex-wrap relative z-10">
                <Link
                  href={`/certificates/${cert.id}`}
